...
class AddViewController_category: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        lblCategory?.text = gCategories[indexPath.row]

        return cell
    }
    
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        category = gCategories[indexPath.row]
        
        guard let parentVC = self.parentVC else { return }
        print("데이터 전송 to parent")
        //parentVC.txtCategory.text = category
        parentVC.category = category
